...
(
	...
	totalVertices_(0), max_order_(0), weighted_orders_(nullptr),
	max_weights_(nullptr), orders_(nullptr),
	indices_(nullptr), totalEdges_(0), edges_(nullptr), weights_(nullptr),
	numColors_(0), colors_(nullptr)
{
	// std::cout << sizeof(GraphElem) << " " << sizeof(nv) << std::endl;
	assert(sizeof(GraphElem) == sizeof(nv) &&
		   "SUMAC is compiled for 64-bit int64_t/double.\n");
	...
	memcpy(indices_, xptr, sizeof(*indices_) * (nv+1));
	memcpy(edges_, inds, sizeof(*edges_) * ne);

	for (int32_t i = 0; i < ne; ++i)
		weights_[i] = static_cast<float>(vals[i]);
