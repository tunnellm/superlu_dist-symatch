...
inline int xLUstruct_t<double>::initSymFactWorkspace()
{
    ...
    if (superlu_acc_offload)
    {
        ...
        symV2TinyAggregateEligible.assign(
            static_cast<size_t>(nsupers), 0);
        symV2TinyAggregateCounts.assign(
            xlu_checked_product(static_cast<size_t>(nsupers),
                                static_cast<size_t>(Pr),
                                "SymFact V2 tiny aggregate counts"),
            0);
        symV2TinyAggregateDispls.assign(
            symV2TinyAggregateCounts.size(), 0);
        symV2TinyAggregateTotals.assign(static_cast<size_t>(nsupers), 0);
        symV2TinyAggregateLogicalMessages.assign(
            static_cast<size_t>(nsupers), 0);
        ...
        {
            if (Pc != 1)
                ABORT("GPU3DV2_TINY_AGGREGATE currently requires Pc=1.");
            if (superlu_cuda_aware_mpi())
                ABORT("GPU3DV2_TINY_AGGREGATE currently requires non-CUDA-aware MPI.");
            if (!superlu_sym_v2_pinned_staging() ||
                !superlu_sym_v2_pinned_staging_pool() ||
                symV2PartnerLHostRecvPoolPinned == NULL)
                ABORT("GPU3DV2_TINY_AGGREGATE requires pooled pinned staging.");
            if (superlu_sym_v2_async_factor())
                ABORT("GPU3DV2_TINY_AGGREGATE requires GPU3DV2_ASYNC_FACTOR=0 to preserve collective order.");
            if (nsupers > static_cast<int_t>(std::numeric_limits<int>::max()))
                ABORT("SymFact V2 tiny aggregate plan exceeds MPI integer counts.");

            int column_size = 0;
            int column_rank = -1;
            if (MPI_Comm_size(grid3d->cscp.comm, &column_size) != MPI_SUCCESS ||
                MPI_Comm_rank(grid3d->cscp.comm, &column_rank) != MPI_SUCCESS)
                ABORT("SymFact V2 tiny aggregate cannot inspect the process-column communicator.");
            if (column_size != Pr || column_rank != myrow)
                ABORT("SymFact V2 tiny aggregate requires process-column ranks to match process rows.");

            std::vector<int> local_send_counts(
                static_cast<size_t>(nsupers), 0);
            std::vector<int> local_message_counts(
                static_cast<size_t>(nsupers), 0);
            std::vector<int> local_participates(
                static_cast<size_t>(nsupers), 0);
            for (int_t k0 = 0; k0 < nsupers; ++k0)
                local_participates[static_cast<size_t>(k0)] =
                    (isNodeInMyGrid[k0] == 1) ? 1 : 0;

            for (int_t lk = 0; lk < local_cols; ++lk)
            {
                int_t k0 = symV2PanelGid(lk);
                if (k0 < 0 || k0 >= nsupers || isNodeInMyGrid[k0] != 1)
                    continue;
                size_t flat = static_cast<size_t>(lk) *
                              static_cast<size_t>(Pc);
                if (flat >= symV2PartnerLSendSizes.size())
                    ABORT("SymFact V2 tiny aggregate send size is missing.");
                ...
                for (int pr = 0; pr < Pr; ++pr)
                {
                    size_t active_pos = flat * static_cast<size_t>(Pr) +
                                        static_cast<size_t>(pr);
                    if (active_pos >= symV2PartnerLSendRowActive.size())
                        ABORT("SymFact V2 tiny aggregate activity map is missing.");
                    if (symV2PartnerLSendRowActive[active_pos] &&
                        PNUM(pr, 0, grid) != iam)
                        ++remote_messages;
                }
                if (remote_messages > 0)
                {
                    local_send_counts[static_cast<size_t>(k0)] = count;
                    local_message_counts[static_cast<size_t>(k0)] =
                        remote_messages;
                }
            }

            const size_t plan_slots = xlu_checked_product(
                static_cast<size_t>(nsupers), static_cast<size_t>(Pr),
                "SymFact V2 tiny aggregate gathered counts");
            std::vector<int> gathered_counts(plan_slots, 0);
            std::vector<int> global_message_counts(
                static_cast<size_t>(nsupers), 0);
            std::vector<int> all_participate(
                static_cast<size_t>(nsupers), 0);

            if (MPI_Allgather(local_send_counts.data(),
                              static_cast<int>(nsupers), MPI_INT,
                              gathered_counts.data(),
                              static_cast<int>(nsupers), MPI_INT,
                              grid3d->cscp.comm) != MPI_SUCCESS)
                ABORT("SymFact V2 tiny aggregate count exchange failed.");
            if (MPI_Allreduce(local_message_counts.data(),
                              global_message_counts.data(),
                              static_cast<int>(nsupers), MPI_INT, MPI_SUM,
                              grid3d->cscp.comm) != MPI_SUCCESS)
                ABORT("SymFact V2 tiny aggregate message-count reduction failed.");
            if (MPI_Allreduce(local_participates.data(),
                              all_participate.data(),
                              static_cast<int>(nsupers), MPI_INT, MPI_MIN,
                              grid3d->cscp.comm) != MPI_SUCCESS)
                ABORT("SymFact V2 tiny aggregate participation reduction failed.");

            const size_t per_message_limit =
                superlu_sym_v2_tiny_aggregate_bytes();
            const size_t total_limit =
                superlu_sym_v2_tiny_aggregate_total_bytes();
            const int min_messages =
                superlu_sym_v2_tiny_aggregate_min_messages();

            for (int_t k0 = 0; k0 < nsupers; ++k0)
            {
                size_t base = static_cast<size_t>(k0) *
                              static_cast<size_t>(Pr);
                size_t total_count = 0;
                size_t max_count = 0;
                for (int pr = 0; pr < Pr; ++pr)
                {
                    int count = gathered_counts[
                        static_cast<size_t>(pr) *
                            static_cast<size_t>(nsupers) +
                        static_cast<size_t>(k0)];
                    if (count < 0)
                        ABORT("SymFact V2 tiny aggregate has a negative count.");
                    if (total_count >
                        static_cast<size_t>(std::numeric_limits<int>::max()) -
                            static_cast<size_t>(count))
                        ABORT("SymFact V2 tiny aggregate count overflows MPI int.");
                    symV2TinyAggregateCounts[base + static_cast<size_t>(pr)] =
                        count;
                    symV2TinyAggregateDispls[base + static_cast<size_t>(pr)] =
                        static_cast<int>(total_count);
                    total_count += static_cast<size_t>(count);
                    max_count = SUPERLU_MAX(max_count,
                                             static_cast<size_t>(count));
                }
                symV2TinyAggregateTotals[static_cast<size_t>(k0)] =
                    static_cast<int>(total_count);
                symV2TinyAggregateLogicalMessages[static_cast<size_t>(k0)] =
                    global_message_counts[static_cast<size_t>(k0)];

                const size_t max_bytes = xlu_checked_product(
                    max_count, sizeof(double),
                    "SymFact V2 tiny aggregate maximum message");
                const size_t total_bytes = xlu_checked_product(
                    total_count, sizeof(double),
                    "SymFact V2 tiny aggregate total payload");
                if (all_participate[static_cast<size_t>(k0)] != 0 &&
                    global_message_counts[static_cast<size_t>(k0)] >=
                        min_messages &&
                    total_count > 0 &&
                    total_count <=
                        static_cast<size_t>(maxSymPartnerLvalCount) &&
                    total_count <= symV2PartnerLHostRecvPoolPinnedCount &&
                    max_bytes <= per_message_limit &&
                    total_bytes <= total_limit)
                    symV2TinyAggregateEligible[static_cast<size_t>(k0)] = 1;
            }
        }
...
inline int xLUstruct_t<double>::freeSymFactWorkspace()
{
    ...
    symV2ExchangeRecvReqsScratch.clear();
    symV2ExchangeSendReqsScratch.clear();
    symV2TinyAggregateEligible.clear();
    symV2TinyAggregateCounts.clear();
    symV2TinyAggregateDispls.clear();
    symV2TinyAggregateTotals.clear();
    symV2TinyAggregateLogicalMessages.clear();
