...
static inline bool superlu_sym_v2_pcfrag_taskflow_force_output_locks()
{
...
static inline bool superlu_sym_v2_pcfrag_taskflow_async_pieces()
{
    return superlu_sym_v2_env_bool_flag("GPU3DV2_PCFRAG_TASKFLOW_ASYNC_PIECES", 1);
}

static inline bool superlu_sym_v2_pcfrag_taskflow_async_core()
{
...
{
    return superlu_sym_v2_env_bool_flag(
        "GPU3DV2_PCFRAG_TASKFLOW_ATOMIC_OUTPUT_SCATTER",
...
static inline bool superlu_sym_v2_pcfrag_taskflow_exact_group_dispatch()
{
    return superlu_sym_v2_env_bool_flag(
        "GPU3DV2_PCFRAG_TASKFLOW_EXACT_GROUP_DISPATCH",
        superlu_sym_v2_pcfrag_taskflow_async_core() ? 1 : 0);
}
...
static inline bool superlu_sym_v2_pcfrag_taskflow_defer_send_post()
{
    return superlu_sym_v2_env_bool_flag(
        "GPU3DV2_PCFRAG_TASKFLOW_DEFER_SEND_POST",
        superlu_sym_v2_pcfrag_taskflow_async_core() ? 1 : 0);
}
...
static inline int superlu_sym_v2_pcfrag_taskflow_piece_max_rows()
{
    static int cached = -1;
    if (cached >= 0)
        return cached;
...
static inline int superlu_sym_v2_pcfrag_taskflow_event_poll_backoff()
{
    ...
    char *end = NULL;
    long value = std::strtol(env, &end, 10);
    if (end == env || *end != '\0' || value < 0 || value > 1024L)
        ABORT("GPU3DV2_PCFRAG_TASKFLOW_EVENT_POLL_BACKOFF must be an integer in [0,1024].");
    cached = static_cast<int>(value);
    return cached;
}

static inline int superlu_sym_v2_pcfrag_taskflow_event_poll_backoff_max()
{
    static int cached = -1;
    if (cached >= 0)
        return cached;
    const char *env =
        std::getenv("GPU3DV2_PCFRAG_TASKFLOW_EVENT_POLL_BACKOFF_MAX");
    if (env == NULL || env[0] == '\0')
    {
    ...
    long value = std::strtol(env, &end, 10);
    if (end == env || *end != '\0' || value < 0 || value > 1024L)
        ABORT("GPU3DV2_PCFRAG_TASKFLOW_EVENT_POLL_BACKOFF_MAX must be an integer in [0,1024].");
    cached = static_cast<int>(value);
    return cached;
}
...
{
    return superlu_sym_v2_env_bool_flag(
        "GPU3DV2_PCFRAG_TASKFLOW_SKIP_UNREQUIRED_POLL",
        superlu_sym_v2_pcfrag_taskflow_async_core() ? 1 : 0);
}
...
static inline int superlu_sym_v2_pcfrag_taskflow_producer_task_limit()
{
    ...
    if (env == NULL || env[0] == '\0')
    {
        cached = superlu_sym_v2_pcfrag_taskflow_async_core() ? 0 : 1;
        return cached;
    }
    char *end = NULL;
    long value = std::strtol(env, &end, 10);
