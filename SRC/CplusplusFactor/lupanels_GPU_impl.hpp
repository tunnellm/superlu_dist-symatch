    ...
    int_t k, int_t stream_offset)
{
    ...
    {
        if (cuda_aware)
            ABORT("GPU3DV2_PC_FRAGMENT_SCHUR prototype currently requires SUPERLU_CUDA_AWARE_MPI=0.");
        if (async_factor)
            ABORT("GPU3DV2_PC_FRAGMENT_SCHUR prototype requires GPU3DV2_ASYNC_FACTOR=0.");
        ...
        if (exact_row_fragment_demand &&
            (symV2RowFragExactSendSizes.empty() ||
             symV2RowFragExactSendBufsGPU.empty() ||
             symV2RowFragExactSendMapsGPU.empty()))
            ABORT("GPU3DV2_EXACT_FRAGMENT_DEMAND buffers are not allocated.");
    }
    ...
    if (pc_fragment_schur)
    {
        ...
        if (!superlu_sym_v2_rowfrag_dest_pack())
        {
        ...
        auto row_pack_destination = [&](int pc_dest) -> int
        {
            if (mycol != kcol_)
            ...
            for (int pc = 0; pc < Pc; ++pc)
            {
                ...
                int size = exact_row_fragment_demand
                    ? symV2RowFragExactSendSizes[active_pos]
                    : symV2PartnerLSendSizes[flat];
                int_t *sendmap = exact_row_fragment_demand
                    ? symV2RowFragExactSendMapsGPU[active_pos]
                ...
                sym_l2u_pack_kernel<<<blocks, threads, 0, stream>>>(
                    ...
                    A_gpu.symV2RowFragStageBufs[stream_offset] + dst_offset,
                    sendmap, size);
                packed_any = true;
            }
            if (packed_any)
        ...
        if (mycol == kcol_)
        {
            for (int pc_dest = 0; pc_dest < Pc; ++pc_dest)
            {
                if (pc_dest == mycol)
                    continue;
                int send_total = row_pack_destination(pc_dest);
                if (send_total <= 0)
                    continue;
                if (static_cast<size_t>(stream_offset) >=
                        symV2RowFragHostRecvBufs.size() ||
        ...
        if (!row_index.empty())
        {
            ...
            {
                int self_total = row_pack_destination(mycol);
            ...
            for (int pc = 0; pc < Pc; ++pc)
            {
                ...
                double *stage = A_gpu.symV2RowFragStageBufs[stream_offset] +
                                offset;
                sym_lfrag_assemble_kernel<<<pieces, 256, 0, stream>>>(
                    stage, A_gpu.symV2RowFragValRecvBufs[stream_offset],
                    row_map_gpu, pieces, ksupc, row_nrows);
