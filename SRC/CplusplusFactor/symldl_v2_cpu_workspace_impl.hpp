...
static size_t symldl_v2_cpu_bounded_memory_per_rank(MPI_Comm comm)
{
    ...
    if (available_pages > 0 && page_size > 0 &&
        static_cast<unsigned long long>(available_pages) <=
            std::numeric_limits<unsigned long long>::max() /
                static_cast<unsigned long long>(page_size))
        available = static_cast<unsigned long long>(available_pages) *
                    static_cast<unsigned long long>(page_size);
    MPI_Comm shared = MPI_COMM_NULL;
    if (MPI_Comm_split_type(comm, MPI_COMM_TYPE_SHARED, 0, MPI_INFO_NULL,
                            &shared) != MPI_SUCCESS)
        available = std::numeric_limits<unsigned long long>::max();
    else
    {
        int local_ranks = 1;
        MPI_Comm_size(shared, &local_ranks);
        MPI_Comm_free(&shared);
        if (available != std::numeric_limits<unsigned long long>::max())
            available /=
                static_cast<unsigned long long>(SUPERLU_MAX(1, local_ranks));
    }

    // Leave room for the symbolic structure and the legacy panel buffers
    // allocated after the CPU scheduler workspace.
    if (available != std::numeric_limits<unsigned long long>::max())
        available = available / 5ULL * 4ULL;
    ...
    return global_available >
               ...
               ? std::numeric_limits<size_t>::max()
               : static_cast<size_t>(global_available);
#else
    (void) comm;
    ...
    xLUstruct_t<Ftype> *lu, int requested, size_t values_per_slot)
{
    ...
    int local_slots = static_cast<int>(SUPERLU_MIN(
        static_cast<size_t>(requested),
        SUPERLU_MIN(local_limit,
                    static_cast<size_t>(std::numeric_limits<int>::max()))));
    int slots = 0;
    ...
    xLUstruct_t<Ftype> *lu)
{
    ...
    if (needs_aggregate)
    {
        ...
        if (needs_partner)
        {
            values_per_slot += lu->symV2CpuPartnerSendCapacity;
            values_per_slot += lu->symV2CpuPartnerRecvCapacity;
        }
        if (needs_row)
        {
