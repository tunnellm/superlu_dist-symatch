...
void
pdgsequ_new(SuperMatrix *A, double *r, double *c, double *rowcnd,
	double *colcnd, double *rowcri, double *colcri, double *amax, int *info, gridinfo_t *grid)
{
    /* Local variables */
    NRformat_loc *Astore;
    double *Aval;
	...
	*amax = 0.;
    ...
    crimax_loc = SUPERLU_MAX(crimax_loc, fabs(1-r[i]));
    }
    ...
    MPI_Allreduce( &crimax_loc, rowcri,
        1, MPI_DOUBLE, MPI_MAX, grid->comm);
    ...
    *amax = rcmax;
	...
	for (i = 0; i < A->nrow; ++i)
	    r[i] = 1. / sqrt( SUPERLU_MIN( SUPERLU_MAX( r[i], smlnum ), bignum ) );
	...
	*rowcnd = SUPERLU_MAX( rcmin, smlnum ) / SUPERLU_MIN( rcmax, bignum );
    ...
    for (i = 0; i < m_loc; ++i) {
	    ...
	    c[jcol] = SUPERLU_MAX( c[jcol], fabs(Aval[j]) );
	}
	++irow;
    }
    ...
    crimax_loc = SUPERLU_MAX(crimax_loc, fabs(1-c[j]));
    }
    ...
    *colcri = crimax_loc;
	...
	/* Invert the scale factors. */
	for (j = 0; j < A->ncol; ++j)
	    c[j] = 1. / sqrt( SUPERLU_MIN( SUPERLU_MAX( c[j], smlnum ), bignum) );
